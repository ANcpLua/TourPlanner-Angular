...
export class TourMapComponent {
  ...
  readonly routeGeometry = input<readonly (readonly [number, number])[] | null>(null);
  ...
  constructor() {
    effect(() => {
      ...
      this.updateRoute(from, to, geometry);
  ...
  private updateRoute(
    ...
    const hasRealGeometry = geometry !== null && geometry.length >= 2;
    const pathLatLngs = hasRealGeometry
      ? geometry.map(([lng, lat]) => this.leaflet.latLng(lat, lng))
      : [fromLatLng, toLatLng];

    this.leaflet.polyline(pathLatLngs, {
      color: '#8d5a21',
      weight: 3,
      dashArray: hasRealGeometry ? undefined : '8 4',
    }).addTo(this.routeLayer);

    this.map.fitBounds(this.leaflet.latLngBounds(pathLatLngs), { padding: [40, 40] });
  }
}
