...
describe('TourViewModel', () => {
  ...
  it('should compute map coordinates and route geometry from route information', async () => {
    const promise = vm.loadTours();
    httpTesting.expectOne(`${baseUrl}api/tour`).flush([sampleTour]);
    await promise;
    ...
    expect(coords!.geometry).toHaveLength(3);
    expect(coords!.geometry![0]).toEqual([16.3738, 48.2082]);
    expect(coords!.geometry![2]).toEqual([13.405, 52.52]);
  });

  it('should return null geometry when route information has no geometry', async () => {
    const tourWithoutGeometry: Tour = {
      ...sampleTour,
      routeInformation: JSON.stringify({
        FromCoordinates: { Latitude: 48.2082, Longitude: 16.3738 },
        ToCoordinates: { Latitude: 52.52, Longitude: 13.405 },
    ...
    const promise = vm.loadTours();
    httpTesting.expectOne(`${baseUrl}api/tour`).flush([tourWithoutGeometry]);
    await promise;

    const coords = vm.mapCoordinates();
    expect(coords).not.toBeNull();
    expect(coords!.geometry).toBeNull();
  });
  ...
  it('should compute editingTour from editingTourId', async () => {
    const promise = vm.loadTours();
    httpTesting.expectOne(`${baseUrl}api/tour`).flush([sampleTour]);
    await promise;
