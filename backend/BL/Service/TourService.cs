...
public class TourService(ITourRepository tourRepository, IMapper mapper, IUserContext userContext) : ITourService
{
    ...
    public IEnumerable<TourDomain> SearchTours(string searchText)
    {
        var tours = tourRepository
            .GetAllTours(userContext.UserId)
            .Select(mapper.Map<TourDomain>);

        if (string.IsNullOrWhiteSpace(searchText)) return [.. tours];

        return [.. tours.Where(t => t.SearchableText.ContainsIgnoreCase(searchText))];
    }
}
